...
* **预处理**：外部文档的数据中，比如PDF中的表格、图片、页眉页脚都可能会引入噪声，需要专门`parser`或者通过OCR转化为文本，（如 `pdfplumber`、`unstructured`）处理。
...
* **Query Rewrite**：用户的原始问题往往口语化、模糊。
  ...
  1. **HyDE（Hypothetical Document Embedding）**：（假设文档嵌入）让 LLM 先生成一个假设性的答案文档，再用这个文档去检索——因为问题向量和答案向量在空间中往往不对齐
...
* faithfulness-忠实度：answer原子陈述能否从chunk中推导出来
* 答案相关性：反向生成问题，计算与query的相似度
* 上下文精度：判断每一个chunk是否对GT有用
* 上下文召回率：每一个GT原子陈述能否在chunk中找到支持
