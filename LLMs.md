...
LoRA（Low-Rank Adaptation）是一种高效微调大语言模型的方法。

**权重矩阵的变化量 ΔW，其实是低秩的**。

整个 ΔW = BA，只在这个低维空间里学习，就像在说："这次微调的本质变化，其实只有 r 个自由度。"

B初始化为0矩阵，学习率比A大

> 所有矩阵，都可以通过奇异值分解，分解给三个矩阵的乘积。
>
> 对于中间矩阵只有斜对角线有值，如果保留其中较大的几个奇异值作为（r秩数），然后就可以用更小的参数来近似ΔW
...
* 处理无效数据：意义空泛或者模板化的文本，需要进行数据清洗
* Document Length Considerations：需要考虑长文档文本
* Machine Generated Text：数据大部分都是从网络上爬取下来的，但是很多都是机器生成的文本、OCR文本等
* 去重：
* 清洗污染数据：预训练语料库与下游任务中标签相同的数据、评估样本中未含有标签的情况
* 毒性和偏见控制
