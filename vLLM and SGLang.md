...
> **RadixAttention (基数树注意力)**：这是SGLang对标PagedAttention的核心创新。它采用**基数树（Radix Tree）**的数据结构来管理KV缓存，能够高效地**自动检测和复用**请求之间共享的公共前缀。在多轮对话、RAG检索等场景中，这可以**避免重复计算历史对话或固定提示词（Prompt）的KV缓存**，极大地减少了预填充的计算量，从而显著降低延迟并提升吞吐量。在某些场景下，实际吞吐量可超越vLLM 2-5倍。
