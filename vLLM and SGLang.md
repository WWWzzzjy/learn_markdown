...
1. 第一，为每个请求预分配最大长度的 KV Cache 空间，实际生成长度往往远小于预分配，内部碎片导致显存利用率只有 20-40%；
1. 第二，空闲块不连续造成外部碎片，总量够但无法分配给新请求；
1. 第三，相同 System Prompt 被每个请求重复存储，无法共享。
...
>
> SGLang会把不同请求的公共前缀转换为 radix tree 的结构，从而在多个请求共享系统提示词、历史对话、模板前缀时，更激进地复用已有 KV cache
