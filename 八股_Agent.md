...
4. 感知模块：理解环境反馈（包括文本、图像、音频、带有JSON格式的工具返回等）
...
5. Memory是 Agent 的一个关键模块。请问如何为 Agent 设计短期记忆和长期记忆系统？可以借助哪些外部工具或技术？
   ...
   * 程序记忆——对应的是 Agent 的**行为策略、能力**。一般固化进 System prompt，通过FT让模型权重学会（工具）
   * 核心技术栈：
...
6. Agent 的记忆写入机制
   ...
   * 常见的写入机制：
     1. 基于规则的触发：用户表达了**偏好**或者完成了**一项重要任务**
     2. 基于摘要的写入：**对话结**束或者达到一定长度后，让LLM对对话进行总结提炼，将关键信息写入存储
