...
6. Agent 的记忆写入机制
   ...
   Agent 的记忆写入机制指的是 Agent 在与环境交互过程中，将信息持久化存储以供后续检索和使用的过程。它让 Agent 能够跨会话、跨任务地积累和利用经验。

   * 常见的写入机制：
     1. 基于规则的触发：用户表达了偏好或者完成了一项重要任务
     2. 基于摘要的写入：对话结束或者达到一定长度后，让LLM对对话进行总结提炼，将关键信息写入存储
     3. 反思机制：Agent 定期回顾反思，总结经验，从中提炼出更高层次的认知或模式
   * 记忆写入的关键设计：
     1. 写什么：不是所有信息都值得记住。写入内容包括：用户偏好、事实性知识、任务结果、经验教训。需要过滤噪声
     2. 存储类型：键值对（Redis）、向量嵌入（语义检索）、知识图谱、自然语言片段
     3. 冲突处理：新旧记忆矛盾时，需要决定是覆盖、合并还是保留并标记时间。经典策略：最后写入 或 带时间戳版本管理
     4. 遗忘机制：基于访问频率衰减、TTL进行淘汰

7. Tool Use是扩展 Agent 能力的有效途径。请解释 LLM 是如何学会调用外部 API 或工具的？（可以从 Function Calling 的角度解释）

   1. **LLM 的本质是文本输入、文本输出。**

   2. **tool use本质是让 LLM 输出结构化的"意图描述"，**由外部系统解析并真正执行，再把结果还给 LLM
...
6. 工程级别的分层 prompt 模板：
   ...
   1. system prompt：最底层的，包含了Agent 的角色定位、安全边界和全局行为约束（包括输出格式规范）

   2. domain prompt：注入的是领域知识和业务逻辑，不同业务线可以有不同的领域层

   3. task prompt：few-shot示例，针对具体任务的指令模板，通常包含结构化的输入槽位和输出格式要求
...
9. 真实生产环境下的 Agent 完整工作链路
   ...
   2. **核心逻辑层**（Core Logic Layer）
      ...
      * **工具执行层**：也会做一次Redis缓存检索，同时工具执行层面需要设计：**超时与重试**（每个工具调用都有超时限制，超时后自动重试或回退到备用方案。不同工具的超时阈值不同）；**沙箱隔离**（代码执行类工具必须在沙箱环境中运行如Docker，防止恶意代码影响宿主系统）；**结果校验**（对工具返回结果进行校验和清洗，防止出现prompt injection，需要做安全过滤）；**工具结果缓存**：对时效性不敏感的调用结果，缓存到 Redis 中避免重复请求
      * **评估与反思**：更加成熟一点的系统中，会引入一个**独立的模型**或者启发式规则对agent的输出做**质量检查**、**安全检查**，如果发现了错误就出发 Reflexion 反思机制，将是失败错误原因分析并追加到下一轮循环的上下文中。
