...
1. 你如何定义一个基于 LLM 的智能体（Agent）？它通常由哪些核心组件构成？
   ...
   * Agent = **LLM（推理核心、决策）+ 感知 + 记忆 + 工具 的自主决策系统**。它不只是被动地回答问题，而是能感知环境、任务规划、记忆检索、调用工具、返回结果这样一个循环工作的智能体
...
3. 在 Agent 的设计中，“规划能力”至关重要。请谈谈目前有哪些主流方法可以赋予 LLM 规划能力？（CoT、Plan-and-Excute、ReAct、Reflexion、ToT、GoT ）
...
5. Memory是 Agent 的一个关键模块。请问如何为 Agent 设计短期记忆和长期记忆系统？可以借助哪些外部工具或技术？
   ...
   * 结构化状态：维护一个结构化的工作区，记录任务状态、中间变量等
   ...
   长期记忆：长期记忆对应 Agent **跨会话、跨任务**的**知识积累**，必须借助**外部存储**实现持久化。 通过RAG 按需取回

   * 语义记忆——知识库的形式：存储用户手册、产品文档、历史报告、背景资料，
   * 情节记忆——历史经历：存储 Agent 过去的完整对话记录、任务执行轨迹、用户偏好，供将来参考
...
11. Agent 评估指标有哪些
    ...
    * 结果类：任务完整率 / Pass@k（代码和搜索场景）
    * 过程类：工具调用准确率 / 错误恢复能力
    * 效率：平均响应时间 / 平均 token 消耗 / 平均工具调用次数 / 
    * 稳定性：多次运行方差 / 对输入改写的鲁棒性

    

12. 对于在真实场景下(金融对话问答)的高并发 Agent 的**缓存优化**

    缓存场景的特殊性：数据实效性强、查询高度重复（System Prompt、RAG 检索结果、工具调用结果）、准确性要求极高、高并发（同一时段大量用户问相似问题（如"今天沪深300涨了多少？"）

    多层缓存架构：缓存优化的本质是**识别请求的可复用边界**

    * **Prompt Cache（LLM 侧）**：vLLM、SGLang等推理引擎都有前缀缓存；Anthropic、OpenAI 都支持 Prompt Caching
...
16. 高并发情况下的一些具体优化策略
    ...
    2. **请求合并**：设定一个时间窗口（100ms），同一时间窗口内发送的相同请求合并对列，只调用一次LLM，将结果广播给所有相同的请求
    ...
    4. **异步预热**：统计日志中每日的Top问题，提前欲生成答案写入缓存，避免高峰期系统压力过大

    

17. Agent系统中 KV Cache 的管理（生命周期）
...
18. **Agent skill** 了解一下，与MCP、tool对区别
...
20. Agent 上线后如何做可观测性（Observability）
...
21. Agent 系统如何做安全防护？常见的风险有哪些？

    * Prompt 注入（用户通过精心构造的输入劫持 Agent 行为）
    * 工具滥用（Agent 被诱导执行危险操作如删除数据）
    * 信息泄漏（Agent 暴露系统 Prompt 或敏感数据）。
    * 防护措施包括：对用户输入做过滤和检测、对工具调用设置权限和审批机制（尤其是写操作）、对输出做内容审核、使用 Guardrails 框架设定行为边界、限制 Agent 的最大执行步数防止无限循环。

22. 将一个 Agent 从原型推向生产环境需要考虑哪些关键问题？
